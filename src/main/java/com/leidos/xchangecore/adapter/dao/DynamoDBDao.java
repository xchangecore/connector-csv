...
public class DynamoDBDao {
    ...
    public void init(String aws_access_key_id, String aws_secret_access_key, String amazon_endpoint,
            String amazon_region, String db_table_name) {

        logger.debug("DynamoDBDao.init: endpoint: " + amazon_endpoint + ", region: " + amazon_region + ", key_id: "
        ...
        if (aws_access_key_id == null || aws_secret_access_key == null || amazon_endpoint == null
                || amazon_region == null || db_table_name == null) {
            return;
        }
    ...
    public boolean deleteEntry(Map.Entry key) {
        ...
        try {
            ...
            DeleteItemSpec deleteItemSpec = new DeleteItemSpec()
                    .withPrimaryKey(new PrimaryKey("title", key.getKey(), "md5hash", key.getValue()));
            table.deleteItem(deleteItemSpec);
            logger.debug("eleteEntry: ... successful ...");
        } catch (Exception e) {
            logger.error("deleteEntry: Title: [" + key.getKey() + "] & MD5hash: [" + key.getValue() + "]: Error: "
                    + e.getMessage());
            return false;
        }
